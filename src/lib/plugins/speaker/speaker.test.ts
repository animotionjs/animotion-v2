...
import type { SpeakerMessage, SpeakerState } from './session';
	...
	describe('embed mode', () => {
		...
		function setupEmbed() {
			...
			vi.stubGlobal('window', {
				location: { search: '?embed&session=animotion:speaker' },
				open: vi.fn()
			});
			const ctx = createContext();
			const plugin = speakerPlugin();
		...
		it('plays the step in place when the presenter is animating it', () => {
			const { manager, channel } = setupEmbed();
		...
		it('does not play a paused or completed step', () => {
			const { manager, channel } = setupEmbed();
		...
		it('does not replay an unchanged playing step', () => {
			const { ctx, manager, channel } = setupEmbed();
		...
		it('plays in place without re-seeking when the presenter starts animating the current step', () => {
			...
			expect(manager.seek).not.toHaveBeenCalled();
			expect(manager.play).toHaveBeenCalledOnce();
		});
